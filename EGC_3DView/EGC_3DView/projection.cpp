...
namespace egc {
    ...
    {
        float abs_w = abs(vertex.w);

        if ((-abs_w <= vertex.x && vertex.x <= abs_w) &&
            (-abs_w <= vertex.y && vertex.y <= abs_w) &&
            (-abs_w <= vertex.z && vertex.z <= abs_w))
            return false;
        return true;
    }
    ...
    {
        egc::vec3 centreOfTriangle = findCenterPointOfTriangle(triangle);
        vec3 directionView = egc::vec3(0.f, 0.f, 0.f) - centreOfTriangle;
    ...
    void displayNormalVectors(egc::vec3& normalVector, egc::vec4& triangleCenter, SDL_Renderer* renderer, egc::mat4 viewTransformMatrix, egc::mat4 perspectiveMatrix)
    {
        ...
        SDL_RenderDrawLine(renderer, triangleCenter.x / triangleCenter.w, triangleCenter.y / triangleCenter.w, secondPoint.x / secondPoint.w, secondPoint.y / secondPoint.w);
    }
}